...
# dplyr filtering (filter on a numeric)
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) 

# Choose multiple conditions to filter (filter on factor)
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))

# Choose a range of conditions of a numeric or integer variable (above was seperated by , here is seperated by :)
...
NTL.Tues.90.99 <- filter(NTL.phys.data, lakename %in% ("Tuesday Lake"), 
                         year4 %in% 1990:1999)

```
Question: Why don't we filter using row numbers?

> Answer: Bc they can change if you sorted!!
