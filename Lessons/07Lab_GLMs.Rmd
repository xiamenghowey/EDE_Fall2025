...
PM2.5byO3regression <- lm(
  data = EPAair,
  PM2.5 ~ Ozone)
summary(PM2.5byO3regression)
...
PM2.5byO3 <- 
ggplot(PM2.5byO3regression, aes(x = Ozone,
                                y = PM2.5))
geom_point()
...
EPAairPM2.5 <- 
  EPAair %>%
  select(mean_Lat:Year) %>%
  na.omit()
EPAairPM2.5Corr <- cor(EPAairPM2.5)
corrplot(EPAairPM2.5Corr, method = "ellipse")
corrplot.mixed(EPAairPM2.5Corr, upper = "ellipse")
...
step(TPAIC)
TPmodel <- lm(data = Paul.naomit, PM2.5 ~ mean_Lat + mean_Lng + Month + Year)
summary(TPmodel)
