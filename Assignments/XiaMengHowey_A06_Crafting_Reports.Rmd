...
```{r project.setup, eval=TRUE, message=FALSE, error=FALSE, results='hide', echo=TRUE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r data.summary, eval=TRUE, echo=FALSE}
...
```{r, echo=FALSE, warning=FALSE, fig.cap= "2018 Monthly Ozone Levels", fig.align='left', fig.dim= c(7,4)}
#convert months to factors from integers in the epa_data dataset
epa_data$Month <- as.factor(epa_data$Month)

#create boxplot of 2018 ozone data
Ozone2018 <- epa_data %>%
  filter(Year == "2018") %>% 
  ggplot(
    mapping = aes(
...
```{r, echo=FALSE, warning=FALSE, fig.cap= "2019 Monthly Ozone Levels", fig.align='left', fig.dim= c(7,4)}
#create boxplot of 2019 ozone data
Ozone2019 <- epa_data %>%
  filter(Year == "2019") %>% 
  ggplot(
    mapping = aes(
